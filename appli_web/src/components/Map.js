...
/*       query = `
    ...
    //    routeCenterLon: this.routeViewport.longitude
      ...
      .then(result => {
        console.log("nos resultats de la requête");
        console.log(result);
...
/************************************* */
       /*        this.tempGeojson.features[0].geometry.coordinates = result.records[0].get("route");

       this.map.getSource("tempGeojson").setData(this.tempGeojson); */
...
/*       this.map.addSource("tempGeojson", {
        type: "geojson",
        data: this.tempGeojson
      }); */
